...
FollowMirrorは、ユーザー自身がダウンロードしたThreadsまたはInstagramの公式データエクスポートをブラウザ内だけで分析し、フォローバックされていないアカウントを確認するChrome拡張機能です。

- ログイン情報、Cookie、トークンは要求しません。
- エクスポートファイルは外部サーバーへ送信されません。
- スクレイピング、自動スクロール、自動フォロー解除は行いません。
- 分析結果と確認済み状態はChromeローカルストレージに保存されます。
- 日本語UI、Chrome Web Store向け日本語説明、プライバシーポリシー日本語版を提供しています。
...
- Meta Accounts Center의 정보 내보내기 화면으로 바로 이동할 수 있게 안내합니다.
- 사용자가 다운로드한 ZIP/JSON 파일을 선택하게 합니다.
- ZIP 내부 또는 JSON 파일에서 팔로워/팔로잉 데이터를 찾습니다.
- 팔로잉 목록에서 팔로워 목록을 뺀 결과를 보여줍니다.
- 사용자가 각 계정을 직접 판단할 수 있도록 프로필 링크와 검토 완료 기록을 제공합니다.
...
2. `가져오기` 탭에서 `Meta Accounts Center export 열기`를 엽니다.
3. Meta Accounts Center에서 `Create export`로 데이터 사본을 요청합니다.
   - 분석할 Threads 또는 Instagram 프로필 선택
   - `Export to device` 선택
   - `JSON` 형식 권장
   - `Followers and following`, `Connections` 또는 전체 정보 선택
   - 기간 선택이 보이면 누락을 줄이기 위해 전체 기간 권장
4. 요청이 `Requested` 상태라면 준비 중입니다. 완료 후 제공되는 ZIP 또는 JSON 파일을 내려받습니다.
   - Accounts Center 링크나 스크린샷은 실제 데이터가 아니므로 분석 입력으로 사용하지 않습니다.
5. 사이드패널에서 파일을 선택합니다.
...
- Meta Accounts Center export: https://accountscenter.instagram.com/info_and_permissions/dyi/
...
- `entrypoints/sidepanel/App.tsx`
  ...
  - `분석 결과` 탭: 팔로잉/팔로워 수, 검토할 계정, 검토 완료
- `lib/exportParser.ts`
